...
class ProjectAIViewModel: ObservableObject {
    ...
    func loadInitialData() async {
        ...
        do {
            // 並列でデータを読み込み
            async let contextTask = loadProjectContext()
            async let insightsTask = loadProjectInsights()
            async let chatHistoryTask = loadChatHistory()
    ...
    func compareWithProjects(_ projectIds: [UUID], type: ProjectComparisonType) async {
        ...
        do {
            let comparison = try await projectAIUseCase.compareProjects(
