...
class SettingsViewModel: ViewModelCapable {
    ...
    @Published var transcriptionMethod: TranscriptionMethod = .local
    @Published var preferredAIProvider: LLMProvider = .openai
    ...
    private func loadSettings() {
        ...
        if let methodRaw = UserDefaults.standard.object(forKey: "transcription_method") as? String,
           let method = TranscriptionMethod(rawValue: methodRaw) {
    ...
    private func saveAISettings(
        ...
        UserDefaults.standard.set(method.rawValue, forKey: "transcription_method")
...
class MockAPIUsageTracker: APIUsageTrackerProtocol {
    ...
    func getMonthlyUsage(for month: Date?) async throws -> MonthlyUsage {
        MonthlyUsage(month: Date(), totalAPICalls: 0, totalTokens: 0, totalCost: 0.0, providerBreakdown: [:])
    }
