...
class LLMService: LLMServiceProtocol {
    ...
    func callAnthropic(request: LLMRequest) async throws -> LLMResponse {
        ...
        guard let apiKey = try await apiKeyManager.getAPIKey(for: .anthropic) else {
            throw LLMServiceError.apiKeyNotFound
        }
    ...
    func callGemini(request: LLMRequest) async throws -> LLMResponse {
        ...
        guard let apiKey = try await apiKeyManager.getAPIKey(for: .gemini) else {
            throw LLMServiceError.apiKeyNotFound
        }
    ...
    func getUsageStats() async throws -> LLMUsageStats {
        ...
        let stats = try await usageTracker.getMonthlyUsage()
        
        var providerBreakdown: [LLMProvider: ProviderUsage] = [:]
        for provider in LLMProvider.allCases {
            let usage = try await usageTracker.getProviderUsage(provider)
    ...
    private func calculateCost(for model: LLMModel, tokens: TokenUsage) -> Double {
