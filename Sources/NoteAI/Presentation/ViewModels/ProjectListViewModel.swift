...
class ProjectListViewModel: ViewModelCapable {
    ...
    func refreshProjects() async {
        await withLoadingNoReturn {
            ...
            if self.searchText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                fetchedProjects = try await self.projectUseCase.getAllProjects()
            } else {
                fetchedProjects = try await self.projectUseCase.searchProjects(query: self.searchText)
            }
            
            self.projects = self.sortProjects(fetchedProjects)
        }
    }
    
    func createProject(name: String, description: String?, coverImageData: Data?) async {
        do {
            _ = try await projectUseCase.createProject(
                name: name,
    ...
    func duplicateProject(_ project: Project, newName: String) async {
        do {
            _ = try await projectUseCase.duplicateProject(
                project.id,
                newName: newName
            )
