...
@MainActor
class EmbeddingService: @preconcurrency EmbeddingServiceProtocol {
    ...
    func getAvailableModels() async -> [EmbeddingModel] {
        ...
        // APIモデルの可用性チェック
        if (try? await apiKeyManager.getAPIKey(for: LLMProvider.openAI(.gpt4))) != nil {
    ...
    private func callOpenAIEmbeddingAPI(request: OpenAIEmbeddingRequest) async throws -> OpenAIEmbeddingResponse {
        ...
        guard let apiKey = try await apiKeyManager.getAPIKey(for: LLMProvider.openAI(.gpt4)) else {
...
// djb2hash extension is defined in Core/Common/RAGTypes.swift
