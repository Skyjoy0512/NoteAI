...
class BaseAPIClient: ProviderAPIClientProtocol, @unchecked Sendable {
    let provider: LLMProvider
    let baseURL: String
    private let session: Session
    internal let keyManager: APIKeyManagerProtocol
    ...
    func makeRequest<T: Codable & Sendable>(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            .responseData { response in
                Task {
                    await self.handleResponse(response, responseType: responseType, continuation: continuation)
                }
            }
        }
    }
...
class OpenAIAPIClient: BaseAPIClient, OpenAIClientProtocol, @unchecked Sendable {
    
    init(keyManager: APIKeyManagerProtocol) {
        super.init(provider: .openAI(.gpt4), keyManager: keyManager)
    }
...
class AnthropicAPIClient: BaseAPIClient, AnthropicClientProtocol, @unchecked Sendable {
    
    init(keyManager: APIKeyManagerProtocol) {
        super.init(provider: .anthropic(.claude3Sonnet), keyManager: keyManager)
    }
...
class GeminiAPIClient: BaseAPIClient, GeminiClientProtocol, @unchecked Sendable {
    
    init(keyManager: APIKeyManagerProtocol) {
        super.init(provider: .gemini(.geminipro), keyManager: keyManager)
    }
